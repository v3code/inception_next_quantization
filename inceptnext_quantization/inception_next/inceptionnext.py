...
class MetaNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mult_gamma = torch.ao.nn.quantized.FloatFunctional()
    ...
    def forward(self, x):
        ...
        if self.gamma is not None:
            ...
            gamma = self.quant(self.gamma.reshape(1, -1, 1, 1))
            x = self.mult_gamma.mul(x, gamma)
        x = self.skip_add.add(self.drop_path(x), shortcut)
        return x
...
def fix_state_dict(model, state_dict):
    for num_stage, stage in enumerate(model.stages):
        for num_block, block in enumerate(stage.blocks):
            state_name = f'stages.{num_stage}.blocks.{num_block}.gamma'
            state_dict[state_name] = state_dict[state_name].reshape(1, -1, 1, 1)

@register_model
def inceptionnext_tiny_quant(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        # fix_state_dict(model, state_dict)
